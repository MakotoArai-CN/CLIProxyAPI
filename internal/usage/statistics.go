// Package usage provides optional in-memory usage aggregation for query endpoints.
package usage
...
// LoggerPlugin aggregates usage records in memory for local query endpoints.
...
func NewLoggerPlugin() *LoggerPlugin {
	return &LoggerPlugin{stats: defaultRequestStatistics}
}
...
func SetStatisticsEnabled(enabled bool) {
	statisticsEnabled.Store(enabled)
}

// StatisticsEnabled reports the current recording state.
func StatisticsEnabled() bool {
	return statisticsEnabled.Load()
}
...
// RequestDetail stores per-request usage data.
...
// TokenStats captures token usage by category.
...
// ModelSnapshot summarises metrics for a model within an API key.
...
// Record ingests a usage record and updates aggregates.
...
// MergeResult reports how many details were imported or skipped.
